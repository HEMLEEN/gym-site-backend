...
const sendToCustomer = asyncHandler(async (eventData) => {
	const { name, contact } = eventData
	let commonFooterDiv = await commonFooter()
	return (`<!DOCTYPE html>
	<html lang="en">
		<head>
		...
		<body>
			<p>Dear, ${name}</p>
			<p>Thank you for choosing our gym! We're excited to have you join our community. Here are the details you need to get started</p>
			<p>We look forward to seeing you at the gym! If you have any further questions, feel free to contact us.</p>
			<p>
			Best regards,
			<p>${gym_owner}<br>${position}<br>${gym_name}<br>${eventDetails.address}<br>${contact}</p>
